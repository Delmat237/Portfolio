      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
          {skillData.map((category, categoryIndex) => (
            <div
              key={category.title}
              className="backdrop-blur-sm bg-white/50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl p-6 hover:bg-white dark:hover:bg-white/10 transition-all duration-300 hover:border-blue-400/30 dark:hover:shadow-blue-400/10 shadow-sm hover:shadow-xl"
            >
              <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-6 text-center border-b border-slate-200 dark:border-white/20 pb-3">
                {category.title}
              </h3>
                    ...
                    <div key={skill.name} className="group">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center space-x-3">
                          <IconComponent className="w-5 h-5 text-blue-600 dark:text-blue-400 group-hover:text-pink-500 transition-colors duration-300" />
                          <span className="text-slate-700 dark:text-white font-medium text-sm">{skill.name}</span>
                        </div>
        ...
        <div className="mt-20">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
            {techData.map((techGroup, index) => (
              <div
                key={techGroup.title}
                className="bg-white/50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl p-4 hover:bg-white dark:hover:bg-white/10 transition-all duration-300 hover:border-blue-400/30 dark:hover:shadow-blue-400/10 shadow-sm hover:shadow-lg"
              >
                <h4 className="text-slate-900 dark:text-white font-semibold mb-3 text-sm text-center border-b border-slate-200 dark:border-white/10 pb-2">
                  {techGroup.title}
                </h4>
                <div className="space-y-2">
                  {techGroup.skills.map((skill, skillIndex) => (
                    <div key={skill.name} className="flex items-center space-x-2 text-slate-600 dark:text-gray-300 text-xs py-1 hover:text-slate-900 dark:hover:text-white transition-colors duration-200 group">
                      <div
                      ...
                      <span className="truncate group-hover:text-slate-900 dark:group-hover:text-white transition-colors duration-200">{skill.name}</span>
                    </div>
                  ))}
                </div>
              </div>
